...
public class AnswerRequestHandler 
    : IRequestHandler<AnswerRequestCommand, string>
{
    ...
    public async Task<string> Handle(AnswerRequestCommand request, CancellationToken cancellationToken)
    {
        var userId = _currentUserServices.GetUserId();
        //get request
        var userRequest = await _unitOfWork.Requests.GetRequestById(request.RequestId, cancellationToken);
        ...
        if (userRequest.RequestFor == RequestFor.Project && request.IsAccepted == true)
        {
            ...
            var members = await _unitOfWork.Requests.GetMemberOfProject(userRequest.ProjectId, cancellationToken);
        ...
        else if(userRequest.RequestFor == RequestFor.Activity && request.IsAccepted == true)
        {
            ...
            var members = await _unitOfWork.Requests.GetMemberOfActivity(userRequest.ActivityId, cancellationToken);
            if (members.Any(x => x.Id == userId))
            {
                _unitOfWork.Requests.DeleteRequest(userRequest);
            }
