...
namespace Core.Application.Features.Projects.QueryHandlers
{
    public class GetRequestsReceivedHandler : ResponseHandler
        , IRequestHandler<GetProjectRequestsReceivedQuery, Response<List<ProjectRequestResponse>>>
    {
        ...
        public async Task<Response<List<ProjectRequestResponse>>> Handle(GetProjectRequestsReceivedQuery request, CancellationToken cancellationToken)
        {
            try
            {
                // get requests with user name 
                var userName = _currentUserServices.GetUserName();
                var requests = await _unitOfWork.Requests.GetProjectRequestsReceived(userName, cancellationToken);
